...
void AVLTree::rightDoubleRot(Node **node){ 
	// Aplicar rotação simples a esquerda no filho de node
	// Aplicar rotação simples a direita em node
...
void AVLTree::leftDoubleRot(Node **node){ 
	// Rotação simples a direita no filhode de node
	// Rotação simples a esquerda em node
...
void AVLTree::leftSimpleRot(Node **node, Node *nodeFather){
	...
	// Atualiza o Pai de node para apontar para o filho correto após balanceamento
	if(nodeFather == NULL) // Nó raiz desbalanceado
...
bool AVLTree::internalInsert(Node **node, Node *nodeFather, int value){
	...
	if( *node == NULL){
		*node = new Node(value, 0, NULL, NULL);
		return true; // Atualiza o fator de balanço do nó acima
	}else{
		Node *pNode  = *node;
		...
		if( value <= pNode->value ){ // Pula para sub-arvore esquerda
			...
			if(updateBalance){
				...
				if(pNode->balance == -1){
					return adjustLeftUnbalanced(node, nodeFather); //Aplica rotação ( o nó vai ficar -2 infringindo a regra AVL)
				}else
				if(pNode->balance == 0){
					pNode->balance = -1;
					return true;
			...
			if(updateBalance){
				...
				if(pNode->balance == 0){
					...
					return adjustRightUnbalanced(node, nodeFather); //Aplica rotação ( o nó vai ficar -2 infringindo a regra AVL)
				}
...
void AVLTree::insert(int value){
	...
	if(treeRoot == NULL){
		treeRoot = new Node(value, 0, NULL, NULL); // Primeiro nó na árvore
	}else{
